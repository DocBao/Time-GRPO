...
def get_sorted_ts(time_list: List[str], global_T_min: torch.Tensor = None) -> torch.Tensor:
    """
    输入5个时间点的字符串列表（支持非标准时间），返回排序后的时间戳张量（shape[5]，保留重复）
    参数：global_T_min - 全量时间的最小值（用于填充无效时间）
    """
    # 校验长度（必须为5个时间点，这是业务要求，保留校验）
    assert len(time_list) == 5, f"必须输入5个时间点，当前为{len(time_list)}个"
...
def adaptive_time_reward_no_deduplicate(
        model_output: List[str],  # 模型输出：5个时间点的字符串列表（支持非标准时间，如'时间1'）
        standard_answer: List[str],  # 标准答案：5个时间点的字符串列表（支持非标准时间）
        all_times: List[str],  # 全量原始时间列表（不做去重，含所有事件时间）
        uniformity_level: str  # 离线检测的均匀性等级（"均匀"/"较均匀"/"不均匀"）
) -> float:
    """
    不去重的自适应时间分布奖励函数（GRPO可微分+容错时间解析）
    ...
    # -------------------------- 2. 转换模型输出和标准答案（容错解析） --------------------------
    model_ts = get_sorted_ts(model_output, global_T_min)  # 模型输出时间戳（shape[5]）
    std_ts = get_sorted_ts(standard_answer, global_T_min)  # 标准答案时间戳（shape[5]）
    ...
    model_intervals = model_ts[1:] - model_ts[:-1]  # 模型间隔（shape[4]）
    std_intervals = std_ts[1:] - std_ts[:-1]  # 标准答案间隔（shape[4]）
